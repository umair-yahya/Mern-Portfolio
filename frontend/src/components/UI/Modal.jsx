...
const Modal = ({activeId, setShowModal}) => {
  ...
  return (
    <div className='w-full h-full fixed top-0 left-0 z-10 bg-headingColor bg-opacity-40'>
        <div className='max-w-[600px] absolute top-1/2 left-1/2 z-20 bg-white rounded-[8px] transform
        -translate-x-1/2 -translate-y-1/2 p-5'>
                ...
                <div className='mt-5 flex items-center gap-3 flex-wrap'>
                    <h4 className='text-headingColor text-[18px] text-[700]'>Technologies:</h4>
                    {
                        portfolio.technologies.map((item,i)=>{
                            return(
                                <span key={i} className='bg-gray-200 py-1 px-2 rounded-[5px] text-[14px] leading-0'>{item}</span>
                            )
                        })
                    }
            ...
            flex items-center justify-center rounded-[20px] leading-0 cursor-pointer'>&times;</button>
        </div>
    </div>
  )
}
